...
class dailyWisdom extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($subscribers as $subscriber) {
            ...
            Log::debug($date->addDays($subscriber->payments->last()->period)->isPast() == true);

            if ($date->addDays($subscriber->payments->last()->period + 1)->isToday()) {
            ...
            } elseif ($date->addDays($subscriber->payments->last()->period)->isPast() == false) {
                if ($subscriber->payments->last()->status == "CAPTURED") {
                    $parameter1 = json_encode(array(
                        "type" => "text",
                        "text" => $wisdom
